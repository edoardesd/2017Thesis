...
#define ssid_2                   "wlan_saltuaria"           //ssid per il collegamento wifi
#define password_2             "antlabpolitecnicomilano"        //password della rete wifi
#define ssid                    "IoTPolimi"           //ssid per il collegamento wifi
#define password            "ZpvYs=gT-p3DK3wb"        //password della rete wifi
#define mqtt_server        "10.172.0.12"      //IP del server mqtt (del server aws)
...
#define REFRESH_RATE            240000
#define MQTT_TENTATIVES         10             //number of tentatives of mqtt tx

//===================================== MQTT TOPICS
//il numero della stanza è da cambiare in base alla stanza considerata in room1, room2 o room3

#define txtopic1           "room2/temperature"
#define txtopic2           "room2/pressure"
#define txtopic3           "room2/humidity"
#define txtopic4           "room2/altitude"
#define txtopic5           "room2/light"
#define txtopic6           "room2/nreal"
#define txtopic7           "room2/nstima"
#define txtopic8           "room2/npeople"
#define txtopic9           "room2/nrandom"
#define txtopic10          "room2/pwr"
#define txtopic11          "room2/alfa"
#define txtopic12          "room2/beta"
#define txtopic13          "room2/iterazione"
#define txtopic14          "room2/statistics/delta"
#define txtopic15          "room2/statistics/deltarndm"
#define rxtopic1           "room2/mqtt_freq"
#define rxtopic2           "room2/statistics/nreal"
#define rxtopic3           "room2/setPwr"
...
unsigned long mqtt_freq = 120000;                    //frequenza di invio dei dati, inizialmente impostata a 5 minuti
...
boolean ricerca(int inizio, int fine) {
  ...
  if (inizio > fine) return 0; //non trovato
    ...
    else if (strncmp_P(addr, (char*)pgm_read_dword(&(mac_table[media])), 6) > 0)
...
boolean mqtt_rx() {
  ...
  Serial.println("mqtt_rx");
  Serial.println("waiting for an update...");
...
*/
  ...
  Serial.println("buf: ");
  Serial.print(buf);
  Serial.println("n_stima");
  Serial.print(nstima);
  
  i = 0;
  if(client.publish(txtopic7, buf)){
    Serial.println("topc 7 inviato");
  }
  while (!client.publish(txtopic7, buf)) {
    client.loop();
    i++;
    Serial.println("topic 7");
    if (i > MQTT_TENTATIVES)  {
...
void loop() {
  ...
  for (i = 0; i < occupancy; i++) {
    CurrentMills = millis();
    if (CurrentMills - Saved[i].Time > REFRESH_RATE)
    {
      for (int j = FINESSE - 1; j >= 0; j--) {
        if (Saved[i].RSSI >= pwrange[j] && Saved[i].rndm == 0) delta[j]--;
        if (Saved[i].RSSI >= pwrange[j] && Saved[i].rndm == 1) deltaRndm[j]--;
  ...
  if (CurrentMills - PrevMills > mqtt_freq || buttonstatus != lastButtonStatus) {
   ...
   if (!mqtt_rx()) {
    ...
    if (SetPower == 1) {
      ...
      Serial.print("N stima: ");
      Serial.println(nstima);
      Serial.println("transmit basic");
      if(mqtt_tx_basic()){
        Serial.print(" SUCCESFULL");
      }else{
        Serial.print(" NOT SUCCESFULL");
